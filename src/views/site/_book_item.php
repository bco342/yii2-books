    ...
    <div class="card-body">
        ...
        <p class="card-text">
            <small class="text-muted">
                <?= Yii::t('app', 'By') ?>: <?= Html::encode($authors) ?>
            </small>
        </p>
        <p class="card-text">
            <?= Html::encode(StringHelper::truncate($model->description, 100)) ?>
        </p>
    </div>

    <div class="card-footer bg-transparent">
        <div class="d-flex justify-content-between align-items-center">
            <small class="text-muted"><?= Yii::t('app', 'Published') ?>
                : <?= Yii::$app->formatter->asDate($model->created_at) ?></small>
